  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "User ["..msg.from.id.."] Blocked :|")
			savelog(msg.from.id.." PM", "User ["..msg.from.id.."] Blocked :|")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "<code>Flooding</code> is not allowed here\n@"..username.."["..msg.from.id.."]\nStatus: User kicked :D")
		else
